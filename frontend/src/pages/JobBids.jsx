      ...
      >
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mt-1">
              <div className="flex items-center mt-1">
                ...
                <span className="text-sm text-slate-600 text-nowrap">{bid.freelancer.successRate}% Success Rate</span>
              </div>
              <div className="flex items-center mt-1">
                <CheckCircle className="text-green-400 mr-1" size={16} />
                <span className="text-sm text-slate-600 text-nowrap">{bid.freelancer.completedJobs} Jobs Completed</span>
              </div>
            </div>
          </div>
        </div>
        <div className="mb-4">
          <p className="text-slate-600">
            ...
            <span className="text-[#12a1e2] cursor-pointer hover:underline" onClick={() => handleViewDetails(bid)}>
              View Details
            </span>
          </p>
        </div>
        ...
        <div className="flex items-center mb-2">
          <Clock className="text-[#12a1e2] mr-2" size={18} />
          <span className="text-slate-600">
            {bid.deliveryTime} {bid.deliveryTimeUnit}
          </span>
        </div>
        <div className="flex items-center mb-4">
          <Paperclip className="text-[#12a1e2] mr-2" size={18} />
          <span className="text-slate-600">{bid.attachments.length} attachment(s)</span>
        </div>
        ...
        <div className="flex justify-between mt-6">
          {bid.offerSent ? (
            ...
            <motion.button
              ...
              className="bg-[#12a1e2] text-white px-6 py-2 rounded-md flex items-center transition duration-300 ease-in-out hover:bg-[#0e8cd4]"
            >
          ...
          <motion.button
            ...
            className="bg-slate-200 text-slate-900 px-6 py-2 rounded-md flex items-center transition duration-300 ease-in-out hover:bg-slate-300"
          >
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={closePopup} className="text-slate-500 hover:text-white">
            <X size={24} />
          </button>
        </div>
        <div className="space-y-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-1">
                <div className="flex items-center">
                  <Star className="text-yellow-400 mr-1" size={16} />
                  <span className="text-sm text-slate-600">{bid.freelancer.successRate}% Success Rate</span>
                </div>
                <div className="flex items-center">
                  <CheckCircle className="text-green-400 mr-1" size={16} />
                  <span className="text-sm text-slate-600">{bid.freelancer.completedJobs} Jobs Completed</span>
                </div>
              </div>
            </div>
          </div>
          <div>
            ...
            <p className="text-slate-600">{bid.proposal}</p>
          </div>
          <div>
            ...
            {bid.milestones && bid.milestones.length > 0 ? (
              <div className="space-y-4">
                {bid.milestones.map((milestone, index) => (
                  <div key={milestone._id} className="bg-slate-100 p-4 rounded-lg">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-slate-500 text-sm">Amount: PKR {milestone.amount}</span>
                      <span className="text-slate-500 text-sm">Due: {new Date(milestone.dueDate).toLocaleDateString()}</span>
                    </div>
                    <div className="mt-2">
                      <span className={`text-sm px-2 py-1 rounded ${milestone.status === 'pending' ? 'bg-yellow-500 text-yellow-900' :
              ...
              <p className="text-slate-600">No milestones specified for this bid.</p>
            )}
          </div>
            ...
            <ul className="list-disc list-inside text-slate-600">
              {bid.attachments.map((attachment, index) => (
                <a
                  ...
                  className="text-[#12a1e2] hover:underline flex items-center"
                >
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="bg-gradient-to-r mb-6 h-[200px] from-[#12a1e2]/20 to-slate-50 border-slate-200 flex items-center"
      >
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full">
            <div>
              ...
              <p className="text-slate-500 mt-1">Manage and track bids for your posted job</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {job?.isCrowdsourced ? (
          <div>
            <div className="mb-6">
              <label htmlFor="role-select" className="block text-sm font-medium text-slate-500 mb-2">
                Select Role
              </label>
              <div className="relative">
                <select
                  ...
                  className="block appearance-none w-full bg-white border border-[#2d2d3a] text-white py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-[#2d2d3a] focus:border-[#9333EA]"
                >
